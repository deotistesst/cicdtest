#!/bin/bash
FLAG_FILE="/home/ec2-user/mobile/update_flag"
JAR_FILE="/home/ec2-user/mobile/mobile_project.jar"

if [ -f "$FLAG_FILE" ]; then
    echo "Home 프로젝트 변경 감지. 재기동을 시작합니다."
    
    # 1. 기존 프로세스 종료 (stop_home.sh 호출 또는 직접 종료)
    #PIDS=$(pgrep -f "mobile_project.jar")
    #if [ -n "$PIDS" ]; then
    #    kill -15 $PIDS
    #    sleep 5
    #fi
    ...
    cd /home/ec2-user
    echo "java17 test ok" >> java17_test.txt
